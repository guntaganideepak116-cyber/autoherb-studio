...
import { X, User, Phone, Mail, Briefcase, Clock, Paperclip, MessageSquare, CheckCircle2 } from "lucide-react";
...
export function JobApplicationModal() {
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-end justify-center sm:items-center">
      <div className="absolute inset-0 bg-black/75 backdrop-blur-sm" onClick={() => setOpen(false)} aria-hidden />
      <div
        role="dialog"
        aria-modal="true"
      ...
      >
        ...
        {done ? (
          <div className="py-10 text-center">
            ...
            <p className="mt-4 font-display text-lg font-bold uppercase tracking-wide">Application received</p>
            <p className="mx-auto mt-2 max-w-sm text-sm text-muted-foreground">
              Thank you {values.fullName.trim().split(" ")[0]}. Our team will review your details and
              reach out if there's a fit.
            </p>
            <button
              type="button"
              onClick={() => setOpen(false)}
              className="mt-6 h-12 w-full rounded-full bg-[image:var(--gradient-gold)] text-sm font-semibold text-primary-foreground sm:w-56"
            >
              Close
            </button>
          </div>
          ...
          <>
            ...
            <form onSubmit={onSubmit} noValidate className="mt-6 space-y-4">
              <div>
                ...
                <input id="ap-name" className={fieldClass} value={values.fullName} onChange={set("fullName")} placeholder="Your full name" maxLength={60} autoComplete="name" />
                {err("fullName")}
              </div>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <div>
                  ...
                  <input id="ap-phone" type="tel" inputMode="tel" className={fieldClass} value={values.phone} onChange={set("phone")} placeholder="10-digit mobile" maxLength={16} autoComplete="tel" />
                  {err("phone")}
                </div>
                <div>
                  ...
                  <input id="ap-email" type="email" className={fieldClass} value={values.email} onChange={set("email")} placeholder="you@email.com" autoComplete="email" />
                  {err("email")}
                </div>
              </div>
              ...
              <div>
                ...
                <select id="ap-position" className={fieldClass} value={values.position} onChange={set("position")}>
              ...
              <div>
                ...
                <input id="ap-exp" className={fieldClass} value={values.experience} onChange={set("experience")} placeholder="e.g. 2 years in car detailing" maxLength={80} />
                {err("experience")}
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full rounded-xl border border-border bg-surface/70 p-3 text-sm text-muted-foreground file:mr-3 file:rounded-full file:border-0 file:bg-[image:var(--gradient-gold)] file:px-4 file:py-2 file:text-xs file:font-semibold file:text-primary-foreground focus:border-gold focus:outline-none"
                />
              </div>
              ...
              <div>
                ...
                <textarea id="ap-msg" rows={3} maxLength={500} className="w-full rounded-xl border border-border bg-surface/70 p-3 text-sm text-foreground placeholder:text-muted-foreground/70 focus:border-gold focus:outline-none" value={values.message} onChange={set("message")} placeholder="Tell us a little about yourself" />
                {err("message")}
              </div>
              ...
              <div className="flex gap-3 pt-1 pb-[env(safe-area-inset-bottom)]">
                ...
                <button
                  type="submit"
                  disabled={submitting}
                  className="h-12 flex-[1.6] rounded-full bg-[image:var(--gradient-gold)] text-sm font-semibold text-primary-foreground transition-opacity hover:opacity-90 disabled:opacity-70"
                >
